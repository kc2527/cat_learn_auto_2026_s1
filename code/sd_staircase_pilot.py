# -*- coding: utf-8 -*-
"""
Standalone SD staircase pilot task (no RSA).

Purpose:
- Estimate trial count needed to reach a stable perceptual threshold.
- Reuse no-rotation stimulus space from util_func.make_stim_cats().
"""
...
SEED = 20260318
SOURCE_MODE = "sd_staircase_pilot"
...
if __name__ == "__main__":
    ...
    # Build all different pairs and assign difficulty bins by distance
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            diff_rows.append((i, j, d))
    diff_df = pd.DataFrame(diff_rows, columns=["i", "j", "dist"])

    # Equal-count bins over distance. Higher bin index => harder (smaller distance).
    # First create ascending bins, then invert.
    diff_df["bin_easy_to_hard"] = pd.qcut(
        diff_df["dist"],
        q=N_DIFFICULTY_BINS,
        labels=False,
        duplicates="drop",
    ...
    # Dictionary: bin -> candidate rows
    bin_to_pairs = {}
    for b in sorted(diff_df["difficulty_bin"].unique()):
        bin_to_pairs[int(b)] = diff_df.loc[diff_df["difficulty_bin"] == b].copy().reset_index(drop=True)

    min_bin = int(min(bin_to_pairs.keys()))   # easiest
    max_bin = int(max(bin_to_pairs.keys()))   # hardest
    ...
    try:
        ...
        inst = visual.TextStim(
            win,
            text=(
                "Same/Different Staircase Pilot\n\n"
                "You will see two gratings on each trial.\n"
                f"Press {KEY_SAME.upper()} for SAME, {KEY_DIFF.upper()} for DIFFERENT.\n\n"
                "Difficulty adapts based on your responses.\n"
        ...
        while not run_done:
            ...
            if is_same:
                ...
                pair_pool = bin_to_pairs[level_pre]
                p = pair_pool.iloc[int(rng.integers(0, len(pair_pool)))]
                i = int(p["i"])
                j = int(p["j"])
                stim1 = anchors.iloc[i]
            ...
            if accuracy == 1:
                ...
                if consecutive_correct >= 2:
                    current_level = min(max_bin, current_level + 1)  # harder
                    direction = +1
                    consecutive_correct = 0
            else:
                consecutive_correct = 0
                current_level = max(min_bin, current_level - 1)  # easier
                direction = -1
        ...
        # Fill stop reason on all rows
        for row in trial_rows:
            row["stop_reason"] = stop_reason

        # Threshold summary
        if len(reversal_levels) >= THRESH_FROM_LAST_N_REV:
            thresh_levels = np.array(reversal_levels[-THRESH_FROM_LAST_N_REV:], dtype=float)
        elif len(reversal_levels) > 0:
            thresh_levels = np.array(reversal_levels, dtype=float)
        else:
            thresh_levels = np.array([np.nan], dtype=float)

        threshold_last_mean = float(np.nanmean(thresh_levels))
        threshold_last_sd = float(np.nanstd(thresh_levels))
        ...
        base = f"{participant_id}_{session_label}_{dt_key}"
        trials_path = os.path.join(DATA_DIR, f"sd_staircase_pilot_trials_{base}.csv")
        ...
        summary = pd.DataFrame([{
            "participant_id": participant_id,
            "session_label": session_label,
            "n_trials": int(trial_index),
            "n_reversals": int(reversal_count),
            "threshold_last8_mean": threshold_last_mean,
            "threshold_last8_sd": threshold_last_sd,
            "stop_reason": stop_reason,
            "started_at_iso": run_start_iso,
            "ended_at_iso": run_end_iso,
            "duration_sec": float(duration_sec),
        }])
        summary_path = os.path.join(DATA_DIR, f"sd_staircase_pilot_summary_{base}.csv")
        summary.to_csv(summary_path, index=False)

        # Save staircase trace plot
        trial_df = pd.DataFrame(trial_rows)
        plt.figure(figsize=(10, 4))
        plt.plot(trial_df["trial_index"], trial_df["staircase_level_pre"], lw=1.5, label="Level (pre)")
        if len(reversal_trials) > 0:
            rev_x = np.array(reversal_trials, dtype=int)
            rev_y = np.array(reversal_levels, dtype=float)
            plt.scatter(rev_x, rev_y, s=35, label="Reversals")
        plt.xlabel("Trial")
        plt.ylabel("Staircase Level (higher = harder)")
        plt.title("SD Staircase Trace")
        plt.legend()
        plt.tight_layout()
        plot_path = os.path.join(DATA_DIR, f"sd_staircase_trace_{base}.png")
        ...
        # End screen
            ...
            text=(
                ...
                f"Threshold (last reversals mean): {threshold_last_mean:.3f}\n"
        ...
        print("Pilot stopped by user (ESC).")
    finally:
        win.close()
        core.quit()
